...
describe("Rental cleanup on user deletion", () => {
  ...
  it("should rent a book, delete the user, and verify the book is available again", () => {
    ...
    // Now navigate back to rental screen to verify the book is available again
    cy.visit("http://localhost:3000/");
    cy.get("[data-cy=index_rental_button]").click();
    cy.url().should("include", "/rental");

    // Search for the book that was previously rented
    cy.get("[data-cy=book_search_input]").should("be.visible").type("Dorf");

    // Verify the book is now available (has the rent button visible, not showing as rented)
    cy.get("[data-cy^=book_item_]")
      .first()
      .within(() => {
        // The rent button should be visible, indicating the book is available
